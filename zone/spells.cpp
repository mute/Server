...
*/
	...
	if(IsIllusionSpell(spell_id)
		...
		&& (!spell_target->IsNPC() || spell_target->HasOwner())
		&& IsClient()
...
// single target spells
		...
		case ST_SummonedPet:
		{
			if(!spell_target || !GetPetByID(spell_target->GetID()) ||
				(mob_body != BodyType::Summoned && mob_body != BodyType::Summoned2 && mob_body != BodyType::Summoned3 && mob_body != BodyType::Animal && mob_body != BodyType::SummonedUndead))
			{
...
bool Mob::AddProcToWeapon(uint16 spell_id, bool bPerma, uint16 iChance, uint16 base_spell_id, int level_override, uint32 proc_reuse_time) {
	...
	if (spell_id == SPELL_VAMPIRIC_EMBRACE && !(HasClass(Class::Necromancer))) {
