			...
			if (RuleB(Custom, ServerAuthStats) && InZone() && !CAuthorized) {
				if (CUnauth_tics > 1) {
					if (GetZoneID() != Zones::BAZAAR) {
						...
						Message(Chat::Shout, "You are not using the latest Heroes' Journey client. Visit HeroesJourneyEMU.com for more information. Moving to the Bazaar in %d seconds.", (11 - CUnauth_tics) * 6);
					} else {
						if (CUnauth_tics % 2 == 0) {
...
void Client::BulkSendInventoryItems()
{
	...
	if (delete_no_rent) { //client was offline for more than 30 minutes, delete no rent items
		if (RuleB(Inventory, TransformSummonedBags)) {
			LogDebug("Transforming summoned bags to regular bags for [{}]", GetName());
